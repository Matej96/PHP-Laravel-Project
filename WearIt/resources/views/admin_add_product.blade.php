        ...
        <div class="row col-12">
            ...
            <div class="container col-lg-6 col-md-8">
                <div class="card-body bg-light">
                    ...
                    <form method="POST"
                          action="{{ isset($product_id) ? route('add_product', $product_id) : route('add_product') }}"
                          enctype="multipart/form-data">
                        @csrf

                        <div class="form-outline mb-4">
                            <label class="form-label" for="productName">Názov produktu</label>
                            <input type="text" id="productName" name="productName" class="form-control border-dark"
                                   value="{{ $product ? $product->product_name : old('productName') }}"/>
                            @error('productName')
                            ...
                            <div class="col col-lg-6 d-flex justify-content-center align-items-center">
                                <div class="form-outline mb-4">
                                    ...
                                    <input type="number" id="productPrice" name="productPrice"
                                           class="form-control border-dark number-input"
                                           value="{{ $product ? $product->price : old('productPrice') }}" min="0"
                                           step="0.01"/>
                                    @error('productPrice')
                                    <div class="error">{{ $message }}</div>
                                    @enderror
                        ...
                        <div class="form-outline mb-4">
                            ...
                            <textarea class="form-control border-dark" id="description" name="description"
                                      rows="4">{{ $product ? $product->product_description : old('description') }}</textarea>
                            @error('description')
                            <div class="error">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control border-dark" type="file" id="formFileMultiple"
                                   name="formFileMultiple[]" multiple>
                                        ...
                                        <div class="row align-items-center mb-2">
                                            ...
                                            <div class="col-6">
                                                <button type="button" class="btn btn-danger btn-sm"
                                                        onclick="removeImage(this)">Vymazať
                                                </button>
                                            </div>
                                        </div>
                                    @endforeach
                                    ...
                                    @foreach($variations as $variation)
                                            ...
                                            @if($size->id == $variation->size_id)
                                                <div class="col-6 col-md-3 justify-content-center align-items-center">
                                                    <div class="form-outline mb-4">
                                                        ...
                                                        <input type="hidden" name="productSizes2[]"
                                                               value="{{ $size->id }}">
                                                        <input type="text" id="productSizes2" name="productSizes2[]"
                                                               class="form-control" value="{{ $size->size_name }}"
                                                               disabled>
                                                    </div>
                                                </div>
                                            @endif
                                        ...
                                        <div class="col-6 col-md-3 justify-content-center align-items-center">
                                            <div class="form-outline mb-4">
                                                ...
                                                <input type="number" id="productCount2" name="productCount2[]"
                                                       class="form-control"
                                                       value="{{ old('productCount2.'.$loop->index, (int)$variation->quantity) }}"
                                                       min="0">
                                            </div>
                                            @error('productCount2')
                                            <div class="error">{{ $message }}</div>
                        ...
                        <div class="row">
                            <div class="col col-lg-6 d-flex justify-content-center align-items-center">
                                <div class="form-group mb-4">
                                    ...
                                    <select class="selectpicker border-dark" id="categoryPicker" name="categoryPicker"
                                            title="Kategória" aria-label="size 3 select example">
                                        @foreach($categories as $categories)
                                            <option
                                                value="{{ $categories->id }}" {{ (old('categoryPicker') == $categories->id || ($product && $product->category_id == $categories->id)) ? 'selected' : '' }}>{{ $categories->category_name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col col-lg-6 d-flex justify-content-center align-items-center">
                                <div class="form-group mb-4">
                                    ...
                                    <select class="selectpicker border-dark" data-size="10" id="subCategoryPicker"
                                            title="Podkategória" aria-label="size 3 select example" disabled>
                        ...
                        <div class="row">
                            <div class="col col-lg-6 d-flex justify-content-center align-items-center">
                                <div class="form-group mb-4">
                                    ...
                                    <select class="selectpicker border-dark" id="colorPicker" name="colorPicker"
                                            title="Farba" aria-label="size 3 select example">
                            ...
                            <div class="col col-lg-6 d-flex justify-content-center align-items-center">
                                <div class="form-group mb-4" style="{{ $disableQuantity ? 'display:none;' : '' }}">
                                    ...
                                    <select class="selectpicker border-dark" data-size="10" id="sizePicker"
                                            name="sizePicker[]" title="Veľkosť" multiple
                                            aria-label="size 3 select example">
                                        @foreach($sizes as $size)
                                            <option value="{{ $size->id }}"
                                                    data-content="{{ $size->size_name }}" {{ (collect(old('sizePicker'))->contains($size->id)) ? 'selected':'' }}></option>
                                        @endforeach
                                    </select>
                                    @error('sizePicker')
                                    <div class="error">{{ $message }}</div>
                        ...
                        <div class="col-md-12 d-flex justify-content-center">
                            <button type="submit" name="add_product_button"
                                    class="btn btn-primary btn-block mb-4 add_button">Pridať Produkt
                            </button>
                        </div>
                    </form>
...
@section('customJs')
    <script>
        function removeImage(button) {
            ...
            // Odoslanie AJAX požiadavky na odstránenie obrázku
            fetch('{{ route('image_delete') }}', {
                ...
                .then(response => {
                    if (response.ok) {
                        ...
                        console.error('Chyba pri odstraňovaní obrázka.');
                    }
                })
                .then(data => {
                    // Potom ak sa úspešne vymaže obrázok sa zbytok obrázkov upraví názov
                ...
                .catch(error => {
                    console.error('Chyba pri odstraňovaní obrázka:', error);
                });
        }
    </script>
